...
def get_set_jofogas(set_num, set_name):
    ...
    # Make a list of all found ad
    all_result = []
    for ad in soup.find_all(class_="contentArea"):
        all_result.append(ad)
    # Skip the fake "lepin" legos
    ...
    price_value_tag = data.find("span", {"class": "price-value"})
...
def get_set_vatera(set_num, set_name):
    ...
    # make a GET request to the Vatera website
    response = requests.get(f'https://www.vatera.hu/listings/index.php?q=lego {corrected_set_num[0]}&ob=2&obd=1&p=1')
    # parse the HTML content
    soup = BeautifulSoup(response.content, 'html.parser')
    ...
    all_result = []
    data= 0
    for ad in soup.find_all(class_="prod-inner-container"):
        all_result.append(ad)
    # Skip the fake "lepin" legos
    for top in all_result:
        data=top
    ...
    # Get the AD's price from the website
    price_value_tag = data.find("span", {"class": "originalVal"})
    # Price with spaces
    price_value_full = price_value_tag.text
    # split up based on spaces
    price_value_correction= price_value_full.split()
    # price_value_full had " " in it that means the price_value_correction has >1 element
    if len(price_value_correction) > 1:
        # put together the elements of price_value_correction
        price_value= price_value_correction[0]+price_value_correction[1]
    #   price_value_full had no " " in it that means the price_value_correction has 1 element
    else:
        price_value= price_value_correction[0]
    # Get the AD's subject from the website
